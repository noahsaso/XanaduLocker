...
const getCode = async () => {
  try {
    ...
    // Update confirm button with code.
    const confirmButton = document.getElementById('rentProductItem')
    if (confirmButton) {
      confirmButton.innerHTML = `CONFIRM (${res.code})`
    }
    return res.code
  } catch (err) {
...
const run = async () => {
  ...
  const codeDisplay =
    document.getElementById('codeDisplay') || document.createElement('div')
...
// Fetch code once confirm button exists.
new Promise(async (resolve) => {
    while (true) {
        if (document.getElementById('rentProductItem')) {
            break
        }
        await sleep(1)
    }
